...
class HERSDiagnosticData:
            ...
            for energy_type in self.energy_types:
                for time_type in self.time_types:
                    for measurement_type in self.measurement_types:
                        if time_type == "hourly":
                            self.data_cache[
                                (energy_type, home_type, time_type, measurement_type)
                            ] = [0] * self.NUMBER_OF_TIMESTEPS
    ...
    def calculate_normalized_energy_consumption(self, system_type, system_index):
        ...
        EC_x = self.get_system_energy_consumption(
            "rated_home", system_type, system_index
        )
        EEC_x = self.get_system_energy_efficiency_coefficient(
            "rated_home", system_type, system_index
        )
        EEC_r = self.get_system_energy_efficiency_coefficient(
            "hers_reference_home", system_type, system_index
        )
    ...
    def calculate_total_reference_home_load(self):
        ...
        REC_system_total = self.calculate_other_end_use_energy_consumption(
            "hers_reference_home"
        )
            ...
            if home_type != "hers_reference_home":
                if fuel_type == "ELECTRICITY":
                    ...
                    self.emissions[home_type] += convert(
                        sum(
                            element_product(
                                ...
                                self.data_cache[
                                    (fuel_type, home_type, "hourly", "energy_use")
                                ],
                            )
                        ),
                        "lb/kWh",
                    ...
                    self.emissions[home_type] += (
                        self.data_cache[(fuel_type, home_type, "annual", "energy_use")]
